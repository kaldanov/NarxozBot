...
public class id025_EditDekanat extends Command {
    User user;
    ...
    public boolean execute() throws TelegramApiException {
        ...
        if (hasContact()){
            ...
            if( userRepo.findAllByRolesContains(new Role(3, "ROLE_DEKANAT")).size() <= 1){//todo
                return COMEBACK;
            }
            long delAdminId = getDelAdminId(updateMessageText);
            User notAdmin = userRepo.findById(delAdminId);
            if (notAdmin != null){
                notAdmin.doNotDekanat();//todo
    ...
    private boolean saveDekanat(String phone) throws TelegramApiException {
        ...
        if(newDekanat.isDekanat()){//todo
            alreadyAdminMessId = sendMessage(153);//todo
            return COMEBACK;
        }
        Role roleAdmin = new Role(3, "ROLE_DEKANAT");//todo
    ...
    private String getListDekanats() {
        ...
        admins.append(getText(154)).append(next).append(next);//todo
        Role role = new Role(3, "ROLE_DEKANAT");//todo
        ...
        admins.append(next).append(next).append(getText(155)); //todo
        adminList.clear();
