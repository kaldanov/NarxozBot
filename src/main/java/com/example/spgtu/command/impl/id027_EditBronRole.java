...
public class id027_EditBronRole extends Command {
    User user;
    ...
    public boolean execute() throws TelegramApiException {
        ...
        if (hasContact()){
            ...
            if( userRepo.findAllByRolesContains(new Role(5, "ROLE_BRON")).size() <= 1){//todo
                return COMEBACK;
            }
            long delAdminId = getDelAdminId(updateMessageText);
            User notAdmin = userRepo.findById(delAdminId);
            if (notAdmin != null){
                notAdmin.doNotBron();//todo
    ...
    private boolean saveDekanat(String phone) throws TelegramApiException {
        ...
        if(newDekanat.isBron()){//todo
            alreadyAdminMessId = sendMessage(159);//todo
            return COMEBACK;
        }
        Role roleAdmin = new Role(5, "ROLE_BRON");//todo
    ...
    private String getListDekanats() {
        ...
        admins.append(getText(160)).append(next).append(next);//todo
        Role role = new Role(5, "ROLE_BRON");//todo
        ...
        admins.append(next).append(next).append(getText(161)); //todo
        adminList.clear();
