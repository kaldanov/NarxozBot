...
public class id026_EditOnayRole extends Command {
    User user;
    ...
    public boolean execute() throws TelegramApiException {
        ...
        if (hasContact()){
            ...
            if( userRepo.findAllByRolesContains(new Role(4, "ROLE_ONAY")).size() <= 1){//todo
                return COMEBACK;
            }
            long delAdminId = getDelAdminId(updateMessageText);
            User notAdmin = userRepo.findById(delAdminId);
            if (notAdmin != null){
                notAdmin.doNotOnay();//todo
    ...
    private boolean saveDekanat(String phone) throws TelegramApiException {
        ...
        if(newDekanat.isOnay()){//todo
            alreadyAdminMessId = sendMessage(156);//todo
            return COMEBACK;
        }
        Role roleAdmin = new Role(4, "ROLE_ONAY");//todo
    ...
    private String getListDekanats() {
        ...
        admins.append(getText(157)).append(next).append(next);//todo
        Role role = new Role(4, "ROLE_ONAY");//todo
        ...
        admins.append(next).append(next).append(getText(158)); //todo
        adminList.clear();
